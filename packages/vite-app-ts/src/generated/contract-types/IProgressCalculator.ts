/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import {
  BaseContract,
  BigNumber,
  BigNumberish,
  BytesLike,
  CallOverrides,
  PopulatedTransaction,
  Signer,
  utils,
} from 'ethers';
import { FunctionFragment, Result } from '@ethersproject/abi';
import { Listener, Provider } from '@ethersproject/providers';
import { TypedEventFilter, TypedEvent, TypedListener, OnEvent } from './common';

export interface IProgressCalculatorInterface extends utils.Interface {
  contractName: 'IProgressCalculator';
  functions: {
    'getProgress(uint256,uint256)': FunctionFragment;
  };

  encodeFunctionData(functionFragment: 'getProgress', values: [BigNumberish, BigNumberish]): string;

  decodeFunctionResult(functionFragment: 'getProgress', data: BytesLike): Result;

  events: {};
}

export interface IProgressCalculator extends BaseContract {
  contractName: 'IProgressCalculator';
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: IProgressCalculatorInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(eventFilter?: TypedEventFilter<TEvent>): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(eventFilter: TypedEventFilter<TEvent>): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    getProgress(
      classId: BigNumberish,
      nonceId: BigNumberish,
      overrides?: CallOverrides
    ): Promise<
      [BigNumber, BigNumber] & {
        progressAchieved: BigNumber;
        progressRemaining: BigNumber;
      }
    >;
  };

  getProgress(
    classId: BigNumberish,
    nonceId: BigNumberish,
    overrides?: CallOverrides
  ): Promise<
    [BigNumber, BigNumber] & {
      progressAchieved: BigNumber;
      progressRemaining: BigNumber;
    }
  >;

  callStatic: {
    getProgress(
      classId: BigNumberish,
      nonceId: BigNumberish,
      overrides?: CallOverrides
    ): Promise<
      [BigNumber, BigNumber] & {
        progressAchieved: BigNumber;
        progressRemaining: BigNumber;
      }
    >;
  };

  filters: {};

  estimateGas: {
    getProgress(classId: BigNumberish, nonceId: BigNumberish, overrides?: CallOverrides): Promise<BigNumber>;
  };

  populateTransaction: {
    getProgress(classId: BigNumberish, nonceId: BigNumberish, overrides?: CallOverrides): Promise<PopulatedTransaction>;
  };
}
