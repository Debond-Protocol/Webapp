 ...
 */
export const getAllClasses = async (dataContract: BankData, provider: any): Promise<Map<string, any>> => {
  const allClasses = new Map<string, any>();
  const classIds :BigNumber[]= (await dataContract?.getClasses());
  console.log(classIds)
  // temporary
  const results = await getMultiCallResults0(classIds, dataContract, 'getClassFromId', provider);
